...
from rlvr.reward import sequence_accuracy_reward, sequence_frequency_reward, patch_accuracy_reward, \
    patch_frequency_reward, sequence_structure_reward, patch_structure_reward
...
class GRPOTrainer:
    ...
    def get_per_token_logps(self, model, batch, forecasts, target_dim_pred):
        ...
        for i in range(num_var):
            ...
            if i < target_dim_pred:  # Cannot forecast covariates
    ...
    def compute_patch_advantages(self, rewards, pred_mask, num_var, num_token_per_var, type="PRIME"):  # Corresponding to process supervision
        if type == "PRIME":
    ...
    def compute_loss(self, model, batch, epoch, target_dim_pred):
        ...
        per_token_logps, pred_mask_offset = self.get_per_token_logps(model, batch, forecasts, target_dim_pred)
        ref_per_token_logps, _ = self.get_per_token_logps(self.ref_model, batch, forecasts, target_dim_pred)
        ...
        if self.reward_shaping:
            seq_rewards_total = self.shape_rewards(seq_rewards_total, per_token_logps.detach(), unit="patch",
        ...
        patch_advantages = self.compute_patch_advantages(patch_rewards_offset, pred_mask_offset, num_var, num_token_per_var,
                                                         type="PRIME")  # (batch_size*num_generations, seq_len)
