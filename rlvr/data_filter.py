...
def training_data_selection(ts_model, grpo_trainer, dataloader, num_generations, prediction_length, target_dim, device):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(dataloader):
            ...
            picp_50 = compute_coverage_rate(forecasts, target, prediction_length, target_dim, confidence=0.5).item()
            picp_90 = compute_coverage_rate(forecasts, target, prediction_length, target_dim, confidence=0.9).item()
            if picp_50 >= 0.7:
            ...
            anomaly_ratio = compute_anomaly_ratio(target, target_dim, z_score=1.96)  # 95% confidence interval
            if anomaly_ratio >= 0.5:
                anomaly_indices.append(data_idx)

            # Forecastability selection
            forecastability = compute_forecastability(target, target_dim)
