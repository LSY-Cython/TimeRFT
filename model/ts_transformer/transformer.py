...
from model.ts_transformer.attention import GroupedQueryAttention
from model.ts_transformer.ffn import FeedForward, GatedLinearUnitFeedForward, MoEFeedForward
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(
        self,
        x: torch.FloatTensor,  # (batch_size, seq_len, d_model)
        attn_mask: Optional[torch.BoolTensor] = None,  # (batch_size, seq_len, seq_len)
        var_id: Optional[torch.IntTensor] = None,  # (batch_size, seq_len)
        time_id: Optional[torch.IntTensor] = None,  # (batch_size, seq_len)
        centroid: Optional[torch.FloatTensor] = None,  # (num_experts, d_model)
    ) -> torch.FloatTensor:  # (batch_size, seq_len, d_model)
        if self.pre_norm:
            x = x + self._sa_block(
...
class TransformerEncoder(nn.Module):
    def __init__(
        ...
        if not use_moe:
            ...
            )  # if persistent=True, then save buffer in state_dict, and requires_grad=False
