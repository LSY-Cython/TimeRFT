...
from data.ts_typing import BatchedSample, Sample
...
class PadCollate(Collate):
    ...
    def get_sample_id(self, batch: list[Sample]) -> torch.IntTensor:  # (batch_size, seq_len)
...
class PackCollate(Collate):
    ...
    def first_fit_decreasing_bin_packing(
        self,
        batch: list[Sample],
    ) -> tuple[list[list[Sample]], NDArray[np.int_]]:
        """
        ...
        bin_spaces: NDArray[np.int_] = np.full(len(batch), self.max_length)
        packed_batch: list[list[Sample]] = [[]]

        for sample in batch:
            length = len(sample[self.target_field])
            criterion: NDArray[np.bool_] = bin_spaces - length >= 0
    ...
    def get_sample_id(
        self, batch: list[list[Sample]], bin_spaces: NDArray[np.int_]  # (batch_size, )
    ) -> torch.IntTensor:  # (batch_size, seq_len)
        """
