...
from rlvr.grpo_cov import GRPOTrainer
# from rlvr.data_filter import training_data_selection
from rlvr.data_filter_cov import training_data_selection
...
def sft_training_step(batch: dict[str, torch.Tensor], model, loss_func, seq_fields, target_dim_pred) -> torch.Tensor:
    distr = forward(model,
...
def main(cfg, stage, model_scale="small"):
    ...
    if rl_trainer_cfg["data_selection"]:
        ...
        retain_indices, filter_indices = training_data_selection(ts_model, grpo_trainer, filter_dataloader,
            cfg["eval_dataset"]["num_samples"], train_data_cfg["prediction_length"], train_data_cfg["target_dim"],
            args.target_dim_pred, device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch_data in enumerate(train_dataloader):
            ...
            if stage == "sft":
                loss = sft_training_step(batch_data, ts_model, ts_loss_func, seq_fields[0:-1], args.target_dim_pred)
                reward, kl = None, None
            elif stage == "rft":
                if rl_trainer_cfg["data_selection"]:
            ...
            if ((batch_idx+1) % accumulate_grad_steps == 0) or ((batch_idx+1) == num_batches_per_epoch):
                optimizer.step()
                optimizer.zero_grad()
                # lr_scheduler.step()  # step-wise lr scheduling
                end_time = time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target_dim_pred", type=int, default=1)
    args = parser.parse_args()
    print(args)

    # Load experiment configurations
    cfg_path = "configs/entsoe_30T_100%_moirai_moe_1.0_R_small.yaml"
